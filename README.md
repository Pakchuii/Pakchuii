<!-- 全局包裹并开启圆体魔法 -->
<div align="center" style="font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', sans-serif;">

  <!-- Header SVG & Banner -->
  <h1 align="center">
    <img src="https://readme-typing-svg.demolab.com?font=M+PLUS+Rounded+1c&weight=600&size=24&duration=4000&pause=1000&color=00AEEF&center=true&vCenter=true&width=480&lines=%E4%BF%A1%E3%81%98%E3%82%8B%E5%BF%83%E3%81%8C%E3%81%82%E3%81%AA%E3%81%9F%E3%81%AE%E9%AD%94%E6%B3%95%20%E2%9C%A8;%E7%9B%B8%E4%BF%A1%E7%9A%84%E5%BF%83%E5%B0%B1%E6%98%AF%E4%BD%A0%E7%9A%84%E9%AD%94%E6%B3%95%20%F0%9F%8C%9F;I%27m%20Pakchuii%20%F0%9F%8E%90" alt="Typing SVG" />
  </h1>
  <p align="center" style="color: #00AEEF; font-weight: bold;">
    <em>「 即使是魔法，也有科学无法解释的可爱之处 💠 」</em>
  </p>
  <img src="showcase.webp" style="width: 100%; border-radius: 12px; box-shadow: 0 6px 16px rgba(0, 174, 239, 0.15); border: 4px double #E0F2FE;" alt="Showcase Banner" />

  <p align="center" style="color: #00AEEF; margin: 15px 0;">
    ✨ ┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈ ✨
  </p>

  <!-- ================= 💻 极繁主义：拟物化终端视窗 ================= -->
  <table width="100%" style="border-collapse: collapse; border: 4px double #38BDF8; border-radius: 12px; background-color: #FAFDFF;">
    <tr>
      <!-- 视窗顶部控制栏 -->
      <td colspan="3" bgcolor="#E0F2FE" style="padding: 10px; border-bottom: 3px double #38BDF8; border-radius: 8px 8px 0 0;">
        <div align="left" style="font-size: 14px; color: #0284C7; font-family: 'Courier New', monospace;">
          &nbsp;&nbsp;<b>🔴 🟡 🟢&nbsp;&nbsp;&nbsp;&nbsp;▶ SCHALE / Terminal / pakchuii.exe</b>
        </div>
      </td>
    </tr>
    <tr>
      <!-- ================= 🎀 左面板：身份、动图与画幅图3 ================= -->
      <td width="23%" align="center" valign="top" style="border-right: 3px double #BAE6FD; padding: 15px;">
        <img src="https://github.com/Pakchuii.png" width="130" style="border-radius:50%; box-shadow: 0 0 15px rgba(0, 174, 239, 0.3); border: 4px dashed #38BDF8; padding: 4px; background-color: #FFF;" />
        <br><br>
        <strong style="font-size: 1.3em; color: #0284C7; background-color: #E0F2FE; padding: 2px 14px; border-radius: 12px; border: 2px solid #7DD3FC;">Pakchuii</strong><br>
        <sub style="color: #64748B;">China 🇨🇳 · CS Student</sub>
        <br><br>
        <img src="https://media.tenor.com/J3t8e_yX3KkAAAAi/arisu-blue-archive.gif" width="75" style="border-radius: 12px; border: 2px dashed #BAE6FD; padding: 2px; background-color: #FFF;" />
        <br><br>
        <a href="http://pakchuii.xyz"><img src="https://img.shields.io/badge/🌐_Blog-SCHALE_Terminal-00AEEF?style=for-the-badge" /></a><br>
        <img src="https://img.shields.io/badge/📧_Mail-Contact_Me-38BDF8?style=for-the-badge" /><br>
        <img src="https://komarev.com/ghpvc/?username=Pakchuii&style=flat-square&color=00AEEF&label=Visitor+Senseis" />
        <br><br>
        <h3 style="color: #00AEEF; margin-bottom: 5px; border-bottom: 2px dotted #BAE6FD; padding-bottom: 5px;">✦ Tags Menu</h3>
        <img src="https://img.shields.io/badge/所属-SCHALE-00AEEF?style=flat-square&logoColor=white" /><br>
        <img src="https://img.shields.io/badge/性格-易怒-C084FC?style=flat-square&logoColor=white" /><br>
        <img src="https://img.shields.io/badge/爱好-摸鱼-F472B6?style=flat-square&logoColor=white" /><br>
        <img src="https://img.shields.io/badge/特征-透明-38BDF8?style=flat-square&logoColor=white" /><br>
        <img src="https://img.shields.io/badge/状态-毕设中...-FBBF24?style=flat-square&logoColor=white" />
        <br><br>
        <hr style="border: 1px dashed #7DD3FC;">
        <h3 style="color: #00AEEF;">✦ Daily Shot</h3>
        <!-- 照片3成功移居左侧！重构平衡感 -->
        <img src="photo3.jpg" width="95%" style="border-radius: 8px; border: 4px solid #FFF; border-bottom: 12px solid #FFF; box-shadow: 0 6px 16px rgba(0,0,0,0.12); margin-top: 5px;" alt="Photo 3" />
      </td>
      <!-- ================= 💻 中面板：多层边框嵌套记录 ================= -->
      <td width="55%" valign="top" style="padding: 15px 25px;">
        <h3 style="color: #00AEEF; margin-top: 0; border-bottom: 3px double #BAE6FD; padding-bottom: 5px;">
          💻 System Log
        </h3>
        <pre lang="yaml" style="background-color: #F8FAFC; border: 2px dashed #7DD3FC; color: #1E3A8A; border-radius: 12px; padding: 15px; font-size: 0.9em; font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', monospace;"><code>name: "Pakchuii"
club: "Computer Science"
status: "大概在摸鱼..."
hobbies:
  - 🎮 ALL系杂食党
  - 🎨 正在学习中
  - ☕ 摄取咖啡因
  - 💻 偶尔敲敲键盘
skills:
  - 擅长深夜思考人生
  - 具有 12H 宅家抗性</code></pre>
        <br>
        <h3 style="color: #00AEEF; border-bottom: 3px double #BAE6FD; padding-bottom: 5px;">
          📊 Terminal Stats
        </h3>
        <p align="center" style="margin:0; padding:0;">
          <img width="98%" src="https://github-readme-stats-eight-theta.vercel.app/api?username=Pakchuii&show_icons=true&hide_border=false&bg_color=FFFFFF&title_color=00AEEF&icon_color=0284C7&text_color=475569&border_color=7DD3FC&border_radius=12" alt="Pakchuii's Stats" />
          <br><br>
          <img width="98%" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=Pakchuii&layout=compact&hide_border=false&bg_color=FFFFFF&title_color=00AEEF&text_color=475569&border_color=7DD3FC&border_radius=12" alt="Top Languages" />
        </p>
        <br>
        <h3 style="color: #00AEEF; border-bottom: 3px double #BAE6FD; padding-bottom: 5px;">
          📈 Activity Streak
        </h3>
        <p align="center" style="margin:0; padding:0;">
          <img width="98%" src="https://streak-stats.demolab.com/?user=Pakchuii&theme=transparent&border=7DD3FC&stroke=38BDF8&ring=00AEEF&fire=0284C7&currStreakNum=00AEEF&sideNums=0284C7&currStreakLabel=00AEEF&sideLabels=0284C7&dates=475569&background=FFFFFF&border_radius=12" alt="Activity Streak" />
        </p>
      </td>
      <!-- ================= 📸 右面板：副屏监视器与相册流 ================= -->
      <td width="22%" valign="top" align="center" style="border-left: 3px double #BAE6FD; padding: 15px;">
        <h3 style="color: #00AEEF; margin-top: 0; border-bottom: 2px dotted #BAE6FD; padding-bottom: 5px;">
          💙 Sync...
        </h3>
        <div align="left" style="font-size: 0.85em; background-color: #FFF; padding: 12px; border-radius: 12px; border: 2px dashed #7DD3FC; color: #334155; margin-bottom: 10px;">
          🎧 <b style="color:#0284C7;">Music 🎵</b><br>&nbsp;&nbsp;<i>Nine Point Eight</i><br><br>
          📺 <b style="color:#0284C7;">Watch 📺</b><br>&nbsp;&nbsp;<i>金牌得主</i><br><br>
          📜 <b style="color:#0284C7;">Task 📋</b><br>&nbsp;&nbsp;修完学分...<br><br>
          🍃 <b style="color:#0284C7;">Fav 🌸</b><br>&nbsp;&nbsp;夏天<br><br>
          ☕ <b style="color:#0284C7;">Coffee Lv</b><br>&nbsp;&nbsp;<code style="background:#E0F2FE; color:#0284C7; padding:1px 3px; border: 1px solid #7DD3FC; border-radius: 4px;">[████▒] 40%</code>
        </div>
        <br>
        <hr style="border: 1px dashed #7DD3FC;">
        <h3 style="color: #00AEEF; line-height: 1.2;">
          📸 Momotalk
        </h3>
        <img src="photo1.jpg" width="95%" style="border-radius: 8px; border: 4px solid #FFF; border-bottom: 12px solid #FFF; box-shadow: 0 6px 16px rgba(0,0,0,0.12); margin-top: 5px; margin-bottom: 12px;" alt="Photo 1" />
        <img src="photo2.jpg" width="95%" style="border-radius: 8px; border: 4px solid #FFF; border-bottom: 12px solid #FFF; box-shadow: 0 6px 16px rgba(0,0,0,0.12);" alt="Photo 2" />
      </td>
    </tr>
  </table>

  <!-- ✨ Snake Contribution Grid Animation ✨ -->
  <p align="center" style="margin: 18px 0 10px 0;">
    <img src="github-contribution-grid-snake.svg" alt="Snake animation" width="100%" style="width: 100%; border-radius: 12px; border: 3px double #7DD3FC; background-color: #FAFDFF; padding: 8px; box-shadow: 0 4px 12px rgba(0, 174, 239, 0.08);" />
  </p>

  <p align="center" style="color: #00AEEF; margin: 15px 0;">
    ✨ ┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈ ✨
  </p>

  <!-- ✨ Footer Video Banner ✨ -->
  <p align="center">
    <!-- 加入豪华加宽边框 -->
    <img src="banner.webp" width="100%" style="border-radius: 12px; border: 4px double #38BDF8; background-color: #FFF; padding: 2px;" alt="Dynamic Video Banner" />
  </p>

  <p align="center">
    <sub style="color: #94A3B8; font-weight: bold;">✦ Welcome to SCHALE Terminal · 相信的心就是你的魔法 ✦</sub>
  </p>

</div>
