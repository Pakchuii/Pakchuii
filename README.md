<!-- Header -->
<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=M+PLUS+Rounded+1c&weight=600&size=24&duration=4000&pause=1000&color=00AEEF&center=true&vCenter=true&width=480&lines=%E4%BF%A1%E3%81%98%E3%82%8B%E5%BF%83%E3%81%8C%E3%81%82%E3%81%AA%E3%81%9F%E3%81%AE%E9%AD%94%E6%B3%95%20%E2%9C%A8;%E7%9B%B8%E4%BF%A1%E7%9A%84%E5%BF%83%E5%B0%B1%E6%98%AF%E4%BD%A0%E7%9A%84%E9%AD%94%E6%B3%95%20%F0%9F%8C%9F;I%27m%20Pakchuii%20%F0%9F%8E%90" alt="Typing SVG" />
</h1>

<p align="center" style="color: #00AEEF; font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', sans-serif; font-weight: bold;">
  <em>「 即使是魔法，也有科学无法解释的可爱之处 💠 」</em>
</p>

<!-- ✨ 展示画幅 ✨ -->
<p align="center">
  <img src="showcase.webp" style="width: 100%; border-radius: 12px; box-shadow: 0 4px 12px rgba(0, 174, 239, 0.15); border: 2px solid #E0F2FE;" alt="Showcase Banner" />
</p>

<p align="center" style="color: #00AEEF;">
  ┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
</p>

<!-- 纯 HTML 三栏表格结构：蔚蓝版 (全局启用圆体样式) -->
<div align="center"><table width="100%" style="border-collapse: collapse; border: none; font-family: 'TsukuARdGothic-Regular', 'Nunito', 'Quicksand', 'YouYuan', '幼圆', sans-serif;"><tr><td width="28%" align="center" valign="top"><br><img src="https://github.com/Pakchuii.png" width="150" style="border-radius:50%; box-shadow: 0 6px 16px rgba(0, 174, 239, 0.2); border: 4px solid #FFFFFF;" /><br><br><strong style="font-size: 1.25em;">Pakchuii</strong><br><sub style="color: #64748B;">China 🇨🇳 · CS Student</sub><br><br><a href="http://pakchuii.xyz"><img src="https://img.shields.io/badge/🌐_Blog-SCHALE_Terminal-00AEEF?style=for-the-badge" /></a><br><img src="https://img.shields.io/badge/📧_Mail-Contact_Me-38BDF8?style=for-the-badge" /><br><br><img src="https://komarev.com/ghpvc/?username=Pakchuii&style=flat-square&color=00AEEF&label=Visitor+Senseis" /></td><td width="42%" valign="top"><h3 style="color: #00AEEF; font-family: 'TsukuARdGothic-Regular', 'YouYuan', '幼圆', sans-serif;">✦ Student Info</h3><pre lang="yaml" style="background-color: #F0F9FF; border: 1px solid #BAE6FD; color: #1E3A8A; border-radius: 8px; padding: 12px; font-size: 0.9em; box-shadow: 0 2px 8px rgba(0,174,239,0.05); font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', monospace;"><code>name: "Pakchuii"
club: "Computer Science"
personality: "INTP-T · 偶尔出没"
status: "大概在摸鱼..."

hobbies:
  - 🎮 ALL系杂食党
  - 🎨 正在学习中
  - ☕ 摄取咖啡因
  - 💻 偶尔敲敲键盘

skills:
  - 擅长深夜思考人生
  - 具有 12H 宅家抗性
    </code></pre></td><td width="30%" valign="top"><h3 style="color: #00AEEF; font-family: 'TsukuARdGothic-Regular', 'YouYuan', '幼圆', sans-serif;">✦ Current Sync...</h3><p>🎧 <b>Listening to:</b><br>&nbsp;&nbsp;<i>Nine Point Eight 🎵</i></p><p>📺 <b>Watching:</b><br>&nbsp;&nbsp;<i>金牌得主</i></p><p>📜 <b>Commission:</b><br>&nbsp;&nbsp;努力把学分修完...</p><p>🍃 <b>Favorite:</b><br>&nbsp;&nbsp;夏天</p><p>🔋 <b>咖啡摄入LV:</b><br>&nbsp;&nbsp;<code>[████▒▒▒▒▒▒] 40%</code></p></td></tr></table></div>

<p align="center" style="color: #00AEEF;">
  ┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
</p>

<!-- Personality Tags -->
<p align="center">
  <img src="https://img.shields.io/badge/所属-SCHALE-00AEEF?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/性格-随和-C084FC?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/爱好-摸鱼-F472B6?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/特征-透明-38BDF8?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/状态-毕设工作中...-FBBF24?style=for-the-badge&logoColor=white" />
</p>

<!-- ✨ GitHub Stats: Light Theme ✨ -->
<p align="center">
  <img height="155" src="https://github-readme-stats.vercel.app/api?username=Pakchuii&show_icons=true&hide_border=false&bg_color=F6FBFF&title_color=00AEEF&icon_color=0284C7&text_color=475569&border_color=BAE6FD&border_radius=10&count_private=true" />
  <img height="155" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Pakchuii&layout=compact&hide_border=false&bg_color=F6FBFF&title_color=00AEEF&text_color=475569&border_color=BAE6FD&border_radius=10" />
</p>

<p align="center" style="color: #00AEEF;">
  ┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
</p>

<!-- 🎀 Momotalk Gallery (纪念相册) -->
<h3 align="center" style="color: #00AEEF; font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', sans-serif;">✦ Momotalk Gallery ✦</h3>
<p align="center">
  <img src="photo1.jpg" width="31%" style="border-radius: 12px; border: 2px solid #E0F2FE; box-shadow: 0 4px 8px rgba(0,174,239,0.1);" alt="Gallery Image 1" />&nbsp;
  <img src="photo2.jpg" width="31%" style="border-radius: 12px; border: 2px solid #E0F2FE; box-shadow: 0 4px 8px rgba(0,174,239,0.1);" alt="Gallery Image 2" />&nbsp;
  <img src="photo3.jpg" width="31%" style="border-radius: 12px; border: 2px solid #E0F2FE; box-shadow: 0 4px 8px rgba(0,174,239,0.1);" alt="Gallery Image 3" />
</p>

<p align="center" style="color: #00AEEF;">
  ┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✧ 💠 ✧ ┈┈┈┈┈┈┈┈┈┈┈┈┈┈
</p>

<!-- ✨ Footer Video Banner (动态视频横幅) ✨ -->
<p align="center">
  <img src="banner.webp" width="100%" style="border-radius: 12px; border: 2px solid #E0F2FE;" alt="Dynamic Video Banner" />
</p>

<p align="center">
  <sub style="color: #94A3B8; font-family: 'TsukuARdGothic-Regular', 'Nunito', 'YouYuan', '幼圆', sans-serif; font-weight: bold;">✦ Welcome to SCHALE Terminal · Developed with Coffee ✦</sub>
</p>
